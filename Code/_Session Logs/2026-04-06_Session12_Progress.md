...
## Pending / Next Steps
1. **Download FEC bulk data** — individual contributions files for 2000-2024 from fec.gov/data/browse-data/, save to Data/Raw/fec/
2. **Attrition table** — `tab_sample_attrition.tex` was not regenerated by script 05; needs manual update or script creation to reflect corrected disagreement rates
3. **Session log written** — this file
4. **Compile PDF** via Overleaf and visual pass
5. **Commit and push** changes
6. **Run `.\sync_overleaf.ps1`**
7. **13F + FEC matching** — start after first submission; begin with fuzzy match of 13F filer names to FEC contributors filtered by investment occupation codes
...
- Should the attrition table be regenerated by a script, or updated manually?
- Neil to assign script number for the credibility salience test script (currently 22) and the disagreement fix script (currently 22a)
